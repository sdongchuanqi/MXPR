...
class BackBone_Model(nn.Module):
    ...
    def __init__(self, model='/data/cqdong/XPR-master-with-image/xlm-roberta-base', layer_id=8, wo_linear_head=False):
...
class MoCo(nn.Module):
    ...
    def __init__(self,args, K=2048, m=0.999, T=0.04,config=None):
        ...
        # self.register_buffer("queue1", torch.randn(config.hidden_size, K))
        # self.queue1 = nn.functional.normalize(self.queue1, dim=0)
        # self.register_buffer("queue_ptr1", torch.zeros(1, dtype=torch.long))

        
        # self.register_buffer("queue2", torch.randn(config.hidden_size, K))
        # self.queue2 = nn.functional.normalize(self.queue2, dim=0)
        # self.register_buffer("queue_ptr2", torch.zeros(1, dtype=torch.long))

        # self.register_buffer("queue3", torch.randn(config.hidden_size, K))
        # self.queue3 = nn.functional.normalize(self.queue3, dim=0)
        # self.register_buffer("queue_ptr3", torch.zeros(1, dtype=torch.long)) 
    ...
    def forward(self, im_q, im_k,tag):
        """
        Input:
