...
class WordWithContextDatasetWW(Dataset):
    def __init__(self, words, word2context_en, word2context_de, get_txt_dict,max_len=256, model_name='/data/cqdong/XPR-master-with-image/xlm-roberta-base',
        ...
        for w1, w2 in words: 
            if w1 in word2context_en and w2 in word2context_de:
                ...
                if sentence_bag1 and sentence_bag2:
                    if str(i) in self.image_dict :
                        new_tuple = [sentence_bag1[0][0],sentence_bag1[0][1],sentence_bag1[0][2]+' '+self.vocab.eos_token +' '+self.image_dict[str(i)]]
                        sentence_bag1[0]=new_tuple
                        # j=0
                        # for x in sentence_bag1:
                        #     new_tuple =[sentence_bag1[j][0],sentence_bag1[j][1],sentence_bag1[j][2]+' '+self.vocab.eos_token +' '+self.image_dict[str(i)]] 

                        #     sentence_bag1[j]=new_tuple
                        #     j+=1
    ...
    def __getitem__(self, index):
        ...
        sentence_subset_index_w3,sentence_subset_index_w4 = [],[]
        subset_index_w3,subset_index_w4,subset_leng_w3,subset_leng_w4 = [],[],[],[]
        word3 ,word4 = sentences_w1[0][1],sentences_w2[0][1]
        
        for each_sentence_w3,each_sentence_w4 in zip(sentences_w1,sentences_w2):
            if (' '+self.vocab.eos_token +' ') in each_sentence_w3[2]:
                new_after = each_sentence_w3[2].split(' '+self.vocab.eos_token +' ')[0]
            else:
                new_after = each_sentence_w3[2]

            
            s_before_w3, s_word_w3, s_after_w3 = self.str2indices(each_sentence_w3[0]), self.str2indices(each_sentence_w3[1]), self.str2indices(new_after)
            indices_w3, w3_index, w3_length = self._length_cut(s_before_w3, s_word_w3, s_after_w3)
            sentence_subset_index_w3.append(indices_w3)
            subset_index_w3.append(w3_index)
            subset_leng_w3.append(w3_length)
            s_before_w4, s_word_w4, s_after_w4 = self.str2indices(each_sentence_w4[0]), self.str2indices(each_sentence_w4[1]), self.str2indices(each_sentence_w4[2])
            indices_w4, w4_index, w4_length = self._length_cut(s_before_w4, s_word_w4, s_after_w4)
            sentence_subset_index_w4.append(indices_w4)
            subset_index_w4.append(w4_index)
            subset_leng_w4.append(w4_length)
        sentences_indexs3,sentences_indexs4 = torch.tensor(subset_index_w3),torch.tensor(subset_index_w4)
        sentences_length3,sentences_length4 = torch.tensor(subset_leng_w3),torch.tensor(subset_leng_w4)

        return (sentence_subset_index_w1,sentences_indexs1,sentences_length1,word1), (sentence_subset_index_w2,sentences_indexs2,sentences_length2,word2),(sentence_subset_index_w3,sentences_indexs3,sentences_length3,word3), (sentence_subset_index_w4,sentences_indexs4,sentences_length4,word4)
    ...
    def collate(self, sample_tuples):
        ...
        for i in sample_tuples:
            indices_w3 += i[2][0]
            indices_w4 += i[3][0]
        w3_index = [i[2][1] for i in sample_tuples]
        w3_length = [i[2][2] for i in sample_tuples]
        w3_words = [i[2][3] for i in sample_tuples]
        w4_index = [i[3][1] for i in sample_tuples]
        w4_length = [i[3][2] for i in sample_tuples]
        w4_words = [i[3][3] for i in sample_tuples]
        
        # indices_w3 B * [ExampleNum, L]
        # [B, S]
        ...
        return indices_w1, indices_w2,indices_w3, indices_w4, w1_mask, w2_mask,w3_mask, w4_mask, w1_index, w2_index,w3_index, w4_index, w1_length, w2_length,w3_length, w4_length,w1_type,w2_type,w3_type,w4_type
