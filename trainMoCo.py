...
from utilsWord.sentence_process_multimodel_orig import load_words_mapping,WordWithContextDatasetWW, load_word2context_from_tsv
import re
...
def get_txt_dict(path):
    ...
    with open(path,'r') as f:
        # n=len('a photography of ')
        for line in f:
            match = re.match(r'(\d+)	(.*)', line.strip())
            if match:
                i = match.group(1)
                line1 = match.group(2) if match.group(2) else '' 
                dic[i] = ' '+line1
...
def compute_kl_loss(q, p, pad_mask=None):
...
def train_model(model, train_loader):  # Train an epoch
    ...
    for step, batch in enumerate(tk):
        ...
        batch_src1 = [tensors.to(device) for i,tensors in enumerate(batch) if i % 4 == 2]
        batch_trg1 = [tensors.to(device) for i,tensors in enumerate(batch) if i % 4 == 3]
        with autocast():
            output0, output1,output_0 = model(batch_src,batch_trg)
            loss1, acc1 = lossFunc(output0, output1)
            output2, output3,output_1 = model(batch_trg,batch_src)
            loss2, acc2 = lossFunc(output2, output3)

            output4, output5,output_2 = model(batch_src1,batch_trg1)
            loss3, acc3 = lossFunc(output4, output5)

            output6, output7,output_3 = model(batch_trg1,batch_src1)
            loss4, acc4 = lossFunc(output6, output7)


            t=0.1
            loss1_kl=compute_kl_loss(output_2/t,output_0/t)
            loss2_kl=compute_kl_loss(output_3/t,output_1/t)

        loss = loss1+loss2+loss3+loss4+loss1_kl+loss2_kl
        #loss = loss1 + loss2
        with open('./ano_record.txt','a+')  as f:
            f.write("STEP : " + str(step) + '\n')
            f.write(str(loss1) + " | "+  str(loss2) + '\n' + str(loss1_kl) + " | "+  str(loss2_kl) + '\n')
        acc = (acc1 + acc2)/2
        loss = loss / 4
...
def test_model_single_encoder(model, val_loader):
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(tk):
            ...
            batch_trg = [tensors.to(device) for i,tensors in enumerate(batch) if i % 4 == 1]     
            # batch_src1 = [tensors.to(device) for i,tensors in enumerate(batch) if i % 4 == 2]
            # batch_trg2 = [tensors.to(device) for i,tensors in enumerate(batch) if i % 4 == 3]
            if args.distributed:
...
if __name__ == '__main__':
    ...
    train_image_dict_src = get_txt_dict('/data/cqdong/mPLUG-Owl-main/en-'+args.lg+'.train.imcaption.1')
    valid_image_dict_src = get_txt_dict('/data/cqdong/mPLUG-Owl-main/en-'+args.lg+'.dev.imcaption.1')
    test_image_dict_src = get_txt_dict('/data/cqdong/mPLUG-Owl-main/en-'+args.lg+'.test.imcaption.1')
 

    
    train_dataset = WordWithContextDatasetWW(train_phrase_pairs, en_word2context, lg_word2context,train_image_dict_src,prepend_bos=with_span_eos,append_eos=with_span_eos,sampleNum=args.train_sample_num,
        max_len=args.sentence_max_len)
    dev_dataset =WordWithContextDatasetWW(dev_phrase_pairs, en_word2context, lg_word2context,valid_image_dict_src,prepend_bos=with_span_eos,append_eos=with_span_eos,sampleNum=args.dev_sample_num,
        max_len=args.sentence_max_len) 

    test_dataset = WordWithContextDatasetWW(test_phrase_pairs, en_word2context, lg_word2context,test_image_dict_src,prepend_bos=with_span_eos,append_eos=with_span_eos,sampleNum=args.dev_sample_num,
        max_len=args.sentence_max_len)
    ...
    train_loader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.train_batch_size,collate_fn=train_dataset.collate,drop_last=True,num_workers=16)
    ...
    scheduler = get_linear_schedule_with_warmup(optimizer, len(
        train_loader) // args.gradient_accumulation_steps,
                                                args.num_train_epochs * len(
                                                    train_loader) // args.gradient_accumulation_steps)
    if args.distributed:
        model = DDP(model, device_ids=[args.local_rank], output_device=args.local_rank,broadcast_buffers=False,find_unused_parameters=True)
